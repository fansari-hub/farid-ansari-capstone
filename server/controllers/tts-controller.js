...
if (!req.body.text || typeof req.body.text !== "string") {
  res.status(400).json({ error: "Must provide a text String" });
  return false;
}

if (!req.body.voice || typeof req.body.voice !== "string") {
  res.status(400).json({ error: "You must provide a voice selectiion in string format. Valid choices are : alloy, echo, fable, onyx, nova, and shimme" });
  return false;
}

if (!req.body.messageID || typeof req.body.messageID !== "string") {
  res.status(400).json({ error: "You must a mesasge ID!" });
  return false;
}
